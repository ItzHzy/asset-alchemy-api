...
export class UserDataSource extends FirestoreDataSource<UserDoc, Object> {
	...
	async isFollowing(userId: string, ticker: string) {
		const data = await this.findOneById(userId, {
			ttl: 0,
		});
	...
	async getPlanType(userId: string) {
		const data = await this.findOneById(userId);
	...
	async hasTag(userId: string, tag: string) {
		const data = await this.findOneById(userId);
	...
	async followCompany(userId: string, ticker: string) {
	...
	async unfollowCompany(userId: string, ticker: string) {
		await this.updateOnePartial(userId, {
